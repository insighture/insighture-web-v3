	...
	return (
		...
		>
			...
			<div className="mt-[40px] lg:mt-[64px] flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 lg:gap-0 overflow-hidden">
				...
				{/* Left: featured post — full-width on mobile, bleeds to left edge on desktop */}
				{featured && (
					<Link
						href={`/blog/${featured.slug}`}
						className="group relative mx-4 lg:mx-0 h-[240px] sm:h-[320px] lg:h-[500px] w-[calc(100%-32px)] lg:w-[52%] rounded-[16px] lg:rounded-l-none lg:rounded-r-[16px] overflow-hidden shadow-sm block shrink-0"
					>
						{featured.image ? (
							<DirectusImage
								uuid={featured.image}
			...
			{/* Bottom editorial text — pulled from the featured post */}
			{(featured?.title || featured?.description) && (
				<Container className="mt-[32px] pb-[48px] lg:pb-[80px] flex flex-col gap-[16px] lg:px-[120px]">
					{featured.title && (
						<p className="font-sans font-normal text-[22px] lg:text-[32px] leading-[32px] lg:leading-[48px] text-[#1d2939]">
							{featured.title}
						</p>
					)}
					{featured.description && (
						<p className="font-sans font-normal text-[15px] lg:text-[16px] leading-[24px] lg:leading-[26px] text-[#1d2939] max-w-[665px]">
							{featured.description}
						</p>
					)}
				</Container>
