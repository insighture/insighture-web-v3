...
const NavigationBar = forwardRef<HTMLElement, NavigationBarProps>(({ navigation, globals }, ref) => {
	...
	const effectiveHoverColor = scrolled
		? contextColors.scrolledTextHoverColor ||
			scrolledTextHoverColor ||
			contextColors.textHoverColor ||
			textHoverColor ||
			undefined
	...
	const effectiveActiveTextColor = scrolled
		? contextColors.scrolledActiveTextColor ||
			scrolledActiveTextColor ||
			contextColors.activeTextColor ||
			activeTextColor ||
			undefined
		: contextColors.activeTextColor || activeTextColor || undefined;
	const effectiveActiveUnderlineColor = scrolled
		? contextColors.scrolledActiveUnderlineColor ||
			scrolledActiveUnderlineColor ||
			contextColors.activeUnderlineColor ||
	...
	const effectiveCtaBgColor = scrolled
		? contextColors.scrolledCtaBackgroundColor ||
			pageScrolledCtaBgColor ||
			contextColors.ctaBackgroundColor ||
			pageCtaBgColor ||
			'#ffffff'
		: contextColors.ctaBackgroundColor || pageCtaBgColor || '#ffffff';
	const effectiveCtaTextColor = scrolled
		? contextColors.scrolledCtaTextColor ||
			pageScrolledCtaTextColor ||
			contextColors.ctaTextColor ||
			pageCtaTextColor ||
			'#ec2b54'
	...
	return (
		<>
			...
			<header
				ref={ref}
				className={cn(
					'top-0 z-[60] w-full border-b border-white/20 transition-[transform,box-shadow] duration-300 ease-in-out',
										...
										<NavigationMenuItem key={section.id} className="relative group/navitem">
											{section.children && section.children.length > 0 ? (
												<>
													...
													<NavigationMenuContent
														className="nav-dropdown absolute min-w-[200px] rounded-lg shadow-lg p-2"
														style={{
						...
						<div ref={searchContainerRef} className="relative [&_*]:outline-none [&_*]:ring-0 [&_*]:ring-offset-0">
							...
							>
								...
								>
									...
									<span className={cn('hidden lg:inline transition-opacity duration-300', searchOpen && '!hidden')}>
										Search
									</span>
								</button>
							...
							{searchOpen && (searchLoading || searchSearched) && (
								<div className="absolute right-0 top-full mt-2 w-[280px] sm:w-[360px] lg:w-[420px] max-h-[400px] overflow-auto rounded-lg bg-white text-gray-900 shadow-xl border border-gray-200 z-[70]">
									{searchLoading && <p className="py-4 text-sm text-center text-gray-500">Loading...</p>}
											...
											return (
												<div key={section.id}>
													{section.children && section.children.length > 0 ? (
														<Collapsible>
															<CollapsibleTrigger
																className="font-medium text-[15px] hover:opacity-70 w-full text-left flex items-center justify-between focus:outline-none py-1"
